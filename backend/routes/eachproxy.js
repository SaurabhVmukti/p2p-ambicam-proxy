...
router.get('/:proxytype/:proxyname', async (req, res) => {
    const { proxyname, proxytype } = req.params;

    try {
        // Make the request to the external API with the custom agent
        const response = await axios.get(`https://p2p.ambicam.com:7201/api/proxy/${proxytype}/${proxyname}`, {
            headers: {
